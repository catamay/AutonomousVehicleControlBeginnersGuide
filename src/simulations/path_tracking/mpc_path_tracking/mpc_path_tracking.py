...
def main():
    ...
    vis = GlobalXYVisualizer(x_lim, y_lim, TimeParameters(span_sec=30), gif_name="mpc_path_tracking.gif")
    ...
    # create controller instance and state/control constraints
    control_const = [-3,2]
    state_const = np.array([[-1,1], [-3,3],[-np.pi, np.pi], [-3,3], [0,6]])
    mpc = MpcController(spec, control_const, course, horizon=100, state_const=state_const)
